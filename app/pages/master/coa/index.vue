...
import {
  ...
  fetchCoaUpdate,
  fetchCoaDelete
} from '~/services/master/master_coa'
...
const sort = ref<{ id: string, desc: boolean }[]>([{ id: 'ms_coa_created_date', desc: true }])
const perPageOptions = [5, 10, 20, 50]

const showModal = ref(false)
const modalMode = ref<'create' | 'update'>('create')
const selectedData = ref<Coa | null>(null)
const showDeleteModal = ref(false)
const dataToDelete = ref<Coa | null>(null)
...
const refreshData = async (showShimmer = false) => {
  ...
  loading.value = true
  try {
    const sortField = sort.value[0]?.id || 'ms_coa_created_date'
    const sortOrder = sort.value[0]?.desc ? 'desc' : 'asc'

    const response = await fetchCoaList(
      perPage.value,
      currentPage.value,
      searchQuery.value,
      sortField,
...
const openUpdateModal = (row: Coa) => {
  modalMode.value = 'update'
  selectedData.value = { ...row }
  showModal.value = true
}

const openDeleteModal = (row: Coa) => {
...
const handleSave = async (formData: any) => {
  ...
  try {
    ...
    if (modalMode.value === 'create') {
      res = await fetchCoaCreate(formData)
    } else {
      res = await fetchCoaUpdate(formData)
    }

    if (res) {
      toast.add({ title: `COA berhasil ${modalMode.value === 'create' ? 'dibuat' : 'diperbarui'}!`, color: 'success' })
      await refreshData()
    }
...
const handleDelete = async () => {
  if (dataToDelete.value) {
    loading.value = true
    try {
      const success = await fetchCoaDelete(dataToDelete.value.ms_coa_id)
      if (success) {
        toast.add({ title: 'COA berhasil dihapus!', color: 'success' })
        await refreshData()
      }
...
const typeLabel = (type: string | null) => {
  const map: Record<string, string> = { debit: 'Debit', credit: 'Kredit', both: 'Keduanya' }
      ...
      <UDashboardNavbar>
        <template #leading>
          ...
          <div class="flex items-center gap-2">
            <UIcon name="i-heroicons-book-open" class="size-5 text-primary" />
            <h1 class="text-lg font-bold tracking-tight">Chart of Accounts</h1>
          </div>
        </template>
      </UDashboardNavbar>
    </template>
        ...
        <UCard :ui="{ body: 'p-0 sm:p-0' }">
          ...
          <div v-else class="table-wrapper">
            <UTable
              ...
              class="w-full"
            >
              <template #ms_coa_type-cell="{ row }">
                <UBadge color="info" variant="subtle" size="xs">
                  {{ typeLabel(row.original.ms_coa_type) }}
                </UBadge>
              </template>

              <template #ms_coa_status-cell="{ row }">
                <UBadge
                  :color="row.original.ms_coa_status === 1 ? 'success' : 'neutral'"
                  variant="subtle"
                  size="xs"
                >
                  {{ row.original.ms_coa_status === 1 ? 'Aktif' : 'Nonaktif' }}
                </UBadge>
              </template>
